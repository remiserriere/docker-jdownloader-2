...
# Configure sudo to allow app user to run openvpn and kill ONLY (security: restricted to these binaries only)
...
RUN \
    ...
    echo "app ALL=(ALL) NOPASSWD: /bin/kill" >> /etc/sudoers.d/openvpn && \
